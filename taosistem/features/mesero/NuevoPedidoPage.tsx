...
export const NuevoPedidoPage: React.FC<NuevoPedidoPageProps> = ({ initialTipo }) => {
  const router = useRouter();
  const navigate = (path: string) => router.push(path);
  const params = useParams<{ id?: string }>();
  const orderId = params?.id;
  const isEditing = Boolean(orderId);
  const initialType = initialTipo === OrderType.DOMICILIO ? OrderType.DOMICILIO : OrderType.MESA;
